...
def get_shot_elements(shot_id, text, visual):
    ...
    # --- ACT 2: FORENSIC AUTOPSY — THE DIRT DOESN'T LIE (45 - 96) ---
        ...
        char_style = "flat muted colors"
        bg = "dental examination background with dental probe tool"
        action = "magnifying glass inspecting tooth enamel of ancient skull showing perfectly intact surface with zero cavities"
    elif shot_id in [64]:
        char_style = "flat muted colors"
    ...
    # --- ACT 3: A DAY IN THE LIFE & THE 15-HOUR MATH (97 - 179) ---
        ...
        char_style = "hands distributing roasted food to group, flat muted colors"
        bg = "camp gathering circle background"
        action = "hands distributing generous portions of roasted meat and tubers to elders, adults, and children equally"
    elif shot_id in [134]:
    ...
    # --- ACT 4: CULTURE, PLAY & FIRELIGHT (180 - 236) ---
    elif shot_id in [180]:
        ...
        char_style = "flat muted colors"
        bg = "sandy riverbank background"
        action = "macro view of smooth oval river stones carved with neat geometric tally lines and spiral patterns"
    elif shot_id in [185]:
        char_style = "flat muted colors"
        bg = "flat sand ground background"
        action = "polished deer astragalus ankle bones resting on sand like antique gambling dice"
    elif shot_id in [186]:
        char_style = "two stick figures friendly wrestling on grass, flat muted colors"
        bg = "sunny grassy clearing background with circle of spectators"
        action = "two friendly stick figures grappling on grass while tribe members clap and cheer"
    elif shot_id in [187]:
        char_style = "two travelers greeting friendly tribe on ridge warmly, flat muted colors"
        bg = "open grassy hill ridge background with distant camp"
        action = "two travelers walking up hill waving warmly with open arms to greet neighboring tribe"
    elif shot_id in [188]:
        char_style = "flat muted colors"
        bg = "ancient cave wall illuminated by torchlight"
        action = "limestone cave wall covered in dozens of blown red and yellow ochre hand stencils"
    elif shot_id in [189]:
        char_style = "flat muted colors"
        bg = "hand-drawn map background of southern France with limestone gorge"
        action = "illustrated map of southern France with pin landing on the limestone gorges of the Ardèche"
    elif shot_id in [190]:
        char_style = "silhouette of Paleolithic artist holding burning torch, flat muted colors"
        bg = "dark cave mouth entrance background with daylight fading outside"
        action = "silhouette of artist holding a smoking animal-fat torch stepping boldly into dark cavern"
    elif shot_id in [191]:
        char_style = "flat muted colors"
        bg = "subterranean limestone cave interior background with textured rock walls, stalactites, and warm flickering golden torchlight glow"
        action = "cross-section diagram of deep underground cave passage descending 400 meters into earth"
    elif shot_id in [192]:
        char_style = "artist crouching through low limestone tunnel, flat muted colors"
        bg = "subterranean cave background with rough stone tunnel"
        action = "artist crouching through narrow rock tunnel with flickering torch casting long shadows"
    elif shot_id in [193]:
        char_style = "artist holding torch high in grand cavern, flat muted colors"
        bg = "colossal underground cavern chamber background with giant stalactites"
        action = "artist standing in awe inside giant cavern with torchlight revealing massive limestone ceilings"
    elif shot_id in [194]:
        char_style = "flat muted colors"
        bg = "limestone cave rock surface background"
        action = "charcoal cave painting of four lion heads rendered with dramatic shading and anatomical realism"
    elif shot_id in [195]:
        char_style = "flat muted colors"
        bg = "textured rock wall background with natural stone bulge"
        action = "bulging limestone rock contour incorporated naturally into shoulder of painted woolly rhinoceros"
    elif shot_id in [196]:
        char_style = "flat muted colors"
        bg = "cave wall background in warm torchlight"
        action = "cave painting of running bison with overlapping legs creating dynamic motion picture illusion"
    elif shot_id in [197]:
    ...
    # --- ACT 6: THE AGRICULTURAL TRAP (266 - 315) ---
        ...
        char_style = "flat muted colors"
        bg = "four grid panel background"
        action = "four symbols of civilization: wheat sheaf, locked granary door, tax clay tablet, royal crown"
    elif shot_id in [314]:
    ...
    # --- ACT 7: THE FULL-CIRCLE MIRROR (315 - 334) ---
    elif shot_id in [315]:
